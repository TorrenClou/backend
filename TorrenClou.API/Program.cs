...
using Serilog;
using TorrenClou.API.Extensions;
using TorrenClou.API.Filters;
using TorrenClou.Application.Extensions;
using TorrenClou.Infrastructure.Extensions;
using TorrenClou.Infrastructure.Services;

const string ServiceName = "torrenclou-api";
...
//
//   dotnet run --project TorrenClou.API -- --dump-config-schema
if (args.Contains("--dump-config-schema"))
{
    Console.WriteLine(TorrenClou.Core.Configuration.ConfigSchema.ToJson());
    return;
}
...
try
{
    ...
    builder.Services.AddTorrenClouOpenTelemetry(ServiceName, builder.Configuration, builder.Environment, true);
    ...
    builder.Services.AddHostedService<TorrenClou.Infrastructure.Services.HangfireOrphanReaper>();
    ...
    using (var scope = app.Services.CreateScope())
    {
        ...
        try
        {
            var schemaContext = services.GetRequiredService<TorrenClou.Infrastructure.Data.ApplicationDbContext>();
        ...
        else
        {
            try
            {
                var context = services.GetRequiredService<TorrenClou.Infrastructure.Data.ApplicationDbContext>();
    ...
    app.UseHangfireDashboard("/hangfire", new DashboardOptions
    {
        ...
        DashboardTitle = "TorrenClou Jobs"
    });
